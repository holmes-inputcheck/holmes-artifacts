...
range_raw_filenames = ["range_check_100000_snark.txt"]
range_csv_filenames = ["range_100000_spartan_snark.csv"]
jl_raw_filenames = ["jl_100000_snark.txt"]
jl_csv_filenames = ["jl_100000_spartan_snark.csv"]
...
for i in range(len(range_raw_filenames)):
    ...
    w = parse_range_file("snark", int(range_raw_filenames[i].split('_')[2]), f_config_lines)
    f_config.close()
...
for i in range(len(jl_raw_filenames)):
    ...
    f_config_lines = f_config.readlines()
    w = parse_jl_file("snark", int(jl_raw_filenames[i].split('_')[1]), f_config_lines)
    f_config.close()
