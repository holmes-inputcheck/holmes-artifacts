...
files = ["t_test_p_graph_dataset1.csv", "z_test_p_graph_dataset1.csv", "f_test_p_graph_dataset1.csv", "chi_test_p_graph_dataset1.csv"]
for file in files:
...
normaltest = ax.plot(dc.EntriesChanged, dc.ActualPvalue, color='pink', linewidth=6, label=r"Normalized $\mathbf{\chi^2}$")
...
plt.legend(ncol=1, labelspacing=0.2, columnspacing=0.1, fontsize=24, loc = [0.43, 0.37])

plt.xticks(np.arange(0, threshold + 1, 2000), fontsize=33)
#ax.xaxis.set_major_formatter(mtick.PercentFormatter(num_entries, decimals=0))
ax.xaxis.set_major_formatter(mtick.PercentFormatter(num_entries, symbol=None, decimals=0))

fig = plt.gcf()
fig.set_size_inches(8,6, forward=True)

pp = PdfPages('real_dataset_stat_graph.pdf')
