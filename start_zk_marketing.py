...
print("Starting ZK marketing dataset bench.")
...
for i in range(2):
    cmd = ("scp -i %s -o StrictHostKeyChecking=no zk-marketing.sh ubuntu@%s:~/HOLMES/holmes-library/") % (keyPath, followerPublicAddrs[i])
    process = subprocess.Popen(cmd, shell=True)
    process.wait()

    # kill all zk scripts
    cmd = ("pkill -f zk-marketing -9")
...
party_processes = []
for i in range(2):
    cmd = ("cd ~/HOLMES/holmes-library; chmod 755 zk-marketing.sh; ./zk-marketing.sh %d") % (i + 1)
...
print("Finished ZK marketing dataset benchmarking.")
